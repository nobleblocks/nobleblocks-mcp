...
async def create_literature_review(
    ...
    """Generate a structured literature review on any research topic. Searches the NobleBlocks database for relevant papers, synthesizes findings, and produces a formatted review with inline citations. Accepts 'topic' or 'query' as the research subject."""
    topic = sanitize_input(topic)
    if len(topic) < 5:
        return json.dumps({"error": "Topic must be at least 5 characters"})

    num_papers = max(5, min(num_papers, 50))
    valid_styles = {"narrative", "systematic", "scoping"}
    ...
    try:
        ...
        result = {
            ...
            "content": (data.get("content", {}).get("text") or "")[:3000],
