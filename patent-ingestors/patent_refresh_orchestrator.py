...
# ─── Source 4: Lens.org Scholarly API ─────────────────────────────────────────

LENS_API_KEY = os.environ.get("LENS_API_KEY", "")


def refresh_lens(state, force=False):
    """Pull patent-paper citation links from Lens.org Scholarly API."""
    log.info("─── Lens.org ───")

    if not LENS_API_KEY:
        log.info("LENS_API_KEY not configured — skipping")
        return False
    ...
    try:
        from ingest_lens_scholarly import LensClient, extract_citations_from_work, insert_citations_batch, resolve_dois_to_papers
    except ImportError as e:
        log.error(f"Cannot import ingest_lens_scholarly: {e}")
        return False

    # Get scholarly works cited by patents since last run
    last_date = state.get("lens_last_date")
    if not last_date or force:
        since_date = (datetime.now(timezone.utc) - timedelta(days=30)).strftime("%Y-%m-%d")
    else:
        since_date = last_date

    log.info(f"Fetching Lens.org citations since {since_date}")
    ...
    try:
        ...
        while offset < 10000:  # Lens free tier cap
            result = client.search_patent_cited_works(since_date, offset=offset, size=100)
            if not result:
                break
            ...
            resolve_dois_to_papers(conn, all_dois)

            offset += len(data)
            time.sleep(1.2)  # Rate limit

        conn.close()
        log.info(f"Lens.org: {total_citations} new citations")
        state["lens_last_date"] = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        return total_citations > 0

    except Exception as e:
        log.error(f"Lens ingest error: {e}")
        return False


# ─── Source 5: DOI Resolution ─────────────────────────────────────────────────
...
def main():
    ...
    parser.add_argument("--source", choices=["patentsview", "enriched", "epo", "lens", "resolve", "signals", "all"],
    ...
    try:
        ...
        if args.source in ("lens", "all"):
            results["lens"] = refresh_lens(state, args.force)
